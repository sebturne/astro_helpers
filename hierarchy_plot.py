...
feat_set = 'mstar_smdust_ssfr_hlr_bt'
k = [2,3,4,6]

zscr = N.genfromtxt('Pre/' + feat_set + '_tlz.txt', delimiter = ',')
lbls = N.genfromtxt('../../Data/Clustering/EAGLE/RefL0100N1504_z_0/Post/Labels/' + feat_set + '_tlz_lbls_k%i.txt' % k[0], delimiter = ',')
phi  = N.genfromtxt('Post/Clustering/' + feat_set + '_tlz_staco.txt', delimiter = ',')[:,1:]
phi  = phi[phi[:,1] == k[0],:]
...
ma = E.FS(zscr[:,2], clst[:,0])

fig, ax = MP.subplots()

for j in range(k[0]):
    ax.text(0, ya[j], '$%i$' % (n[ma[j]]), ha = 'center', va = 'center', bbox = dict(facecolor = '#ffffff', edgecolor = '#000000', boxstyle = 'round,pad=0.5'))

ax.text(0, ya[j] + 0.75, '$k = %i$' % k[0], ha = 'center', va = 'center')

for i in range(1,len(k)):

    lbls = N.genfromtxt('../../Data/Clustering/EAGLE/RefL0100N1504_z_0/Post/Labels/' + feat_set + '_tlz_lbls_k%i.txt' % k[i], delimiter = ',')
    phi = N.genfromtxt('Post/Clustering/' + feat_set + '_tlz_staco.txt', delimiter = ',')[:,1:]
    phi = phi[phi[:,1] == k[i],:]
    ...
    for l in range(k[i-1]):
        for j in range(k[i]):
            ...
            ax.arrow(i-1, ya[l], 1, yb[j] - ya[l], fc = '#ff0000', ec = '#ff0000', alpha = o)

    for j in range(k[i]):
        ax.text(i, yb[j], '$%i$' % (n[mb[j]]), ha = 'center', va = 'center', bbox = dict(facecolor = '#ffffff', edgecolor = '#000000', boxstyle ='round,pad=0.5'))
