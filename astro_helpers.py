...
def PH5(hubb):
    ...
    """
    plothubb = N.zeros((N.shape(hubb)[0],1))
...
def PS(im, w, coords):
    ...
    rows = len(im) // w
    grid = N.zeros((rows,w), dtype = N.int)

    for r in range(rows):
        grid[r,:] = range(r,len(im),rows)

    for q in range(len(pics)):
        pics[q] = MX.read_png('../../Data/png/G' + str(int(im[q])) + '.png')
        size = N.shape(pics[q])[0]
        crop = size // 4
        pics[q] = pics[q][crop:size-crop,crop:size-crop]
        boxs[q] = MO.OffsetImage(pics[q], zoom = 0.27)
    
    hpk = [0] * rows

    for r in range(rows):
        hpk[r] = MO.HPacker(children = [boxs[i] for i in grid[r,:]], pad = 0, sep = 0)

    vpk = MO.VPacker(children = [hpk[r] for r in range(rows)], pad = 0, sep = 0)

    pos = coords
    
    return MO.AnnotationBbox(vpk, pos, pad = 0, frameon = False)
...
def GS(vecs):
    ...
    for v in range(vecs.shape[0]):
        w = v - N.sum(N.dot(v,b)*b for b in basis)
        if (w > 1e-10).any():  
            basis.append(w / N.linalg.norm(w))
    return N.array(basis)
...
def H(p, q):
    ...
    """
    return N.sqrt(N.sum((N.sqrt(p) - N.sqrt(q)) ** 2)) / (N.sqrt(2))
